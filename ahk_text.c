...
#define AHK_TEXT_VERSION "0.0.1"
#define AHK_TEXT_TAB_STOP 8
#define AHK_TEXT_QUIT_TIMES 3
...
int editorRowFilePositionXToScreenPositionX(erow *row, int file_position_x) {
  ...
  for (j = 0; j < file_position_x; j++) {
    if (row->characters[j] == '\t')
      screen_position_x += (AHK_TEXT_TAB_STOP - 1) - (screen_position_x % AHK_TEXT_TAB_STOP);
    screen_position_x++;
  }
...
int editorRowScreenPositionXToFilePositionX(erow *row, int screen_position_x) {
  ...
  for (file_position_x = 0; file_position_x < row->size; file_position_x++) {
    if (row->characters[file_position_x] == '\t')
      cur_screen_position_x += (AHK_TEXT_TAB_STOP - 1) - (cur_screen_position_x % AHK_TEXT_TAB_STOP);
...
void editorUpdateRow(erow *row) {
  ...
  row->rendered_characters = malloc(row->size + tabs*(AHK_TEXT_TAB_STOP - 1) + 1);

  int index = 0;
  for (j = 0; j < row->size; j++) {
    if (row->characters[j] == '\t') {
      row->rendered_characters[index++] = ' ';
      while (index % AHK_TEXT_TAB_STOP != 0) row->rendered_characters[index++] = ' ';
    } else {
  ...
  for (y = 0; y < E.screen_rows; y++) {
    ...
    if (filerow >= E.number_of_rows) {
      if (E.number_of_rows == 0) {
        char welcome[120]; // Increased buffer size
        char welcome2[120]; // Increased buffer size

        // Create welcome message
        int welcomelen = snprintf(welcome, sizeof(welcome),
            "AHK editor -- version %s, Written by Paul-philipp Rosenjart.\n", AHK_TEXT_VERSION);

        // Create second welcome message (simplified)
        int welcomelen2 = snprintf(welcome2, sizeof(welcome2),
            "For the ahk kernel."); // Corrected format string

        // Ensure no buffer overflow
        if (welcomelen >= (int)sizeof(welcome)) {
            welcomelen = (int)sizeof(welcome) - 1;
            welcome[welcomelen] = '\0';
        }
        if (welcomelen2 >= (int)sizeof(welcome2)) {
            welcomelen2 = (int)sizeof(welcome2) - 1;
            welcome2[welcomelen2] = '\0';
        }

        if (y == E.screen_rows / 3) {
          // Print the first line of the welcome message
          ...
          // Print the second line of the welcome message
          if (welcomelen2 > E.screen_columns) welcomelen2 = E.screen_columns;
          int padding = (E.screen_columns - welcomelen2) / 2;
          if (padding) {
            abAppend(ab, "~", 2);
            padding--;
          }
          while (padding--) abAppend(ab, " ", 1);
          abAppend(ab, welcome2, welcomelen2);
        } else {
          abAppend(ab, "~", 1);
        }
      } else {
...
void editorProcessKeypress() {
  static int quit_times = AHK_TEXT_QUIT_TIMES;
  ...
  quit_times = AHK_TEXT_QUIT_TIMES;
}
